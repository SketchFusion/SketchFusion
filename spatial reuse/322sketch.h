...
public:
    ...
    struct ApaeSampleEvent {
        int task = -1;
        uint64_t key = 0;
        double est_percentile = -1.0;
        uint64_t window_id = 0;
    };

    std::vector<ApaeSampleEvent> drainApaeSampleEvents();
...
private:
    ...
    void processSharedPSketchC(int task, uint64_t itemKey, const std::string& hashKey);
    ...
    void processExclusivePSketchC(int task, uint64_t itemKey, const std::string& hashKey);
    ...
    void recordApaeSampleEvent(int task, uint64_t key, double est_percentile);

    std::deque<ApaeSampleEvent> apaeSampleEvents;
};
